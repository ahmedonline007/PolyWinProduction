...
const schema = Yup.object({
    description: Yup.string().required("برجاء إدخال وصف التخصيمات").max(200, "الحد الاقصى للكتابة 200 حرف فقط"),
});
...
class Factor extends Component {
    ...
    componentWillReceiveProps(nextState, prevState) {
        if (nextState.ListFactor && nextState.ListFactor.length > 0) {
    ...
    componentDidMount() {
        this.props.actions.GetAllFactor();
    }
    ...
    handleConfirm = () => {
        ...
        this.props.actions.DeleteFactor(this.state.selected);
    }
    ...
    editFactor = (state, rowInfo, column, instance) => {
    ...
    addEditFactor = (value) => {
        ...
        if (value.id > 0) {
            ...
            this.props.actions.AddEditFactor(body);
        } else {
            ...
            <div className="content-page">
                <div className="content">
                    <div className="container-fluid">
                        <div className="row">
                            <div className="col-xl-12">
                                <div className="breadcrumb-holder">
                                    <h1 className="main-title float-left">التخصيمات</h1>
                        ...
                        {/* List Of Data */}
                        <ReactTable
                            getTrProps={this.editFactor}
                            data={this.props.ListFactor}
                            columns={this.cells}
                        />
                    </div>
                </div>
                <Modal show={this.state.show} onHide={this.handleClose.bind(this)}>
                    <Modal.Header closeButton>
                        إضافة أو تعديل التخصيمات
                    </Modal.Header>
                    <Modal.Body className="modal-header">
                        <Formik validationSchema={schema} onSubmit={(values) => { this.addEditFactor(values) }}
                            initialValues={{
                                id: this.state.id,
...
const mapStateToProps = (state, ownProps) => ({
    ListFactor: state.reduces.ListFactor
});

...
const schema = Yup.object({
    catgoryName: Yup.string().required("برجاء إدخال اللون").max(200, "الحد الاقصى للكتابة 200 حرف فقط"),
});


class ParentProductCategory extends Component {
    ...
    componentWillReceiveProps(nextState, prevState) {
        if (nextState.ListParentProductCategory && nextState.ListParentProductCategory.length > 0) {
    ...
    componentDidMount() {
        this.props.actions.getAllParentProductCategory();
    }
    ...
    handleConfirm = () => {
        ...
        this.props.actions.deleteParentProductCategory(this.state.selected);
    }
        ...
        return {
            onClick: (e, handleOriginal) => {
                if (e.target.type !== "checkbox" && (e.target.className === "Edit" || e.target.className === "rt-td")) {
                    ...
                    this.setState({
                        catgoryName: rowInfo.original.catgoryName,
    ...
    addEditParentProductCategory = (value) => {
        this.setState({
            isLoading: true
        });

        let obj = {};
        obj.catgoryName = value.catgoryName;
        obj.Id = value.id;

        this.props.actions.addeditParentProductCategory(obj);
    }
            ...
            <div className="content-page">
                <div className="content">
                    <div className="container-fluid">
                        <div className="row">
                            <div className="col-xl-12">
                                <div className="breadcrumb-holder">
                                    <h1 className="main-title float-left">تسجيل القسم العام للمنتجات</h1>
                        ...
                        <ReactTable
                            ...
                            data={this.props.ListParentProductCategory}
                            columns={this.cells}
                        />
                    </div>
                </div>
                <Modal show={this.state.show} onHide={this.handleClose.bind(this)}>
                    ...
                    <Modal.Body className="modal-header">
                        <Formik validationSchema={schema} onSubmit={(values) => { this.addEditParentProductCategory(values) }}
                            initialValues={{
                                id: this.state.id,
                                catgoryName: this.state.catgoryName
                            }}>
                            {({ handleSubmit, handleChange, handleBlur, values, touched, isValid, errors, }) => (
                                <Form noValidate onSubmit={handleSubmit} style={{ fontWeight: 'bold', fontSize: '25px', width: '100%' }}>
                                    <Form.Group controlId="catgoryName">
                                        ...
                                        <Form.Control type="text"
                                            ...
                                            name="catgoryName"
                                            autoComplete="off"
                                            value={values.catgoryName}
                                            onBlur={handleBlur}
                                            isInvalid={!!errors.catgoryName}
                                        />
...
const mapStateToProps = (state, ownProps) => ({
    ListParentProductCategory: state.reduces.ListParentProductCategory
});
...
export default connect(mapStateToProps, mapDispatchToProps)(ParentProductCategory);
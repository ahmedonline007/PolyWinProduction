...
const schema = Yup.object({
    description: Yup.string().required("برجاء إدخال وصف الملف").max(200, "الحد الاقصى للكتابة 200 حرف فقط"),
});


class PriceLst extends Component {
    ...
    componentWillReceiveProps(nextState, prevState) {
        if (nextState.ListPriceLst && nextState.ListPriceLst.length > 0) {
    ...
    componentDidMount() {
        this.props.actions.GetAllPriceLst();
    }
    ...
    handleConfirm = () => {
        ...
        this.props.actions.DeletePriceLst(this.state.selected);
    }
    ...
    editPriceLst = (state, rowInfo, column, instance) => {
    ...
    addEditPriceLst = (value) => {
        ...
        if (value.id > 0) {
            ...
            this.props.actions.AddEditPriceLst(body);
        } else {
            ...
            <div className="content-page">
                <div className="content">
                    <div className="container-fluid">
                        <div className="row">
                            <div className="col-xl-12">
                                <div className="breadcrumb-holder">
                                    <h1 className="main-title float-left">قائمة الاسعار</h1>
                        ...
                        {/* List Of Data */}
                        <ReactTable
                            getTrProps={this.editPriceLst}
                            data={this.props.ListPriceLst}
                            columns={this.cells}
                        />
                    </div>
                </div>
                <Modal show={this.state.show} onHide={this.handleClose.bind(this)}>
                    <Modal.Header closeButton>
                        إضافة أو تعديل قائمه الاسعار
                    </Modal.Header>
                    <Modal.Body className="modal-header">
                        <Formik validationSchema={schema} onSubmit={(values) => { this.addEditPriceLst(values) }}
                            initialValues={{
                                id: this.state.id,
...
const mapStateToProps = (state, ownProps) => ({
    ListPriceLst: state.reduces.ListPriceLst
});
...
export default connect(mapStateToProps, mapDispatchToProps)(PriceLst);